        ...
        if msg_length:
            ...
            if msg[0:3] == "PUT":
                ...
                try:
                    ...
                    conn.send("Data in wrong format".encode(FORMAT))
                finally:
                    ...
                    for j in receivedData:
                        for keys in j:
                            if (myTrie.search(keys) == False):
                                ...
                                pass
                    conn.send("Data uploaded to server 1".encode(FORMAT))
            elif msg[0:3] == "GET":
                msg=str(msg)
                if (msg[3] != " "):
                    ...
                    conn.send("NOT FOUND, This data is not in server 1".encode(FORMAT))
                ...
                if (msg[5] != " "):
                    ...
                    if Locator == None:
                        conn.send(" NOT FOUND, This data is not in server, 1 please put in a valid key or path".encode(FORMAT))
                    #print(getPersonNumber())
                        ...
                        def sendOutResultsAfterChecking(string):
                            check = string
                            string = eval(string)
                            string = str(string)
                            print(string)
                        ...
                        for i in range(4):
                            try:
                                sendOutResultsAfterChecking(string)
                                break
                            except:
                                conn.send("Data not found..or something went wrong".encode(FORMAT))

            elif msg[0:6] == "DELETE":
                if (msg[6] != " "):
                    conn.send("Please leave space between the query and key/path".encode(FORMAT))
                else:
                    ...
                    if location is None:
                        print("Data is not in Server 1")
                        conn.send("NOT FOUND, Data is not in server 1".encode(FORMAT))
                        print(receivedData)
                    else:
                        receivedData.pop(location)
                        print(receivedData)
                        myTrie.delete(message)
                        conn.send(" ".join([msg[7:len(msg)]+"  has been deleted from server 1", ""]).encode(FORMAT))
